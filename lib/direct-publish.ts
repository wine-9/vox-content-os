...
async function publishSocial(j:DirectJob,platform:'xiaohongshu'|'douyin'){const status=localPublisherStatus();if(DRY_RUN)throw Error('当前为测试模式，真实发布已锁定');if(!status.sau.installed)throw Error('social-auto-upload 未安装');const auth=await checkSauAccount(platform);if(!auth.valid)throw Error(`${platform==='douyin'?'抖音':'小红书'}登录态失效，请先刷新登录`);const tags=allTags(j),args:string[]=[platform];if(j.kind==='video'){const video=j.files.find(x=>x.role==='video'&&x.ready);if(!video)throw Error('视频文件缺失');const cover=await ensureVideoCover(j);args.push('upload-video','--account',ACCOUNT,'--file',video.path,'--title',j.title,...(j.body?['--desc',j.body]:[]),...(tags.length?['--tags',tags.join(',')]:[]),'--thumbnail',cover,'--headless')}else{const images=j.files.filter(x=>x.role==='image'&&x.ready).map(x=>x.path);if(!images.length)throw Error('图片文件缺失');args.push('upload-note','--account',ACCOUNT,'--images',...images,'--title',j.title,...(j.body?['--note',j.body]:[]),...(tags.length?['--tags',tags.join(',')]:[]),'--headless')}const{stdout,stderr}=await run(SAU_BIN,args,{cwd:SAU_CWD,timeout:15*60*1000,maxBuffer:4*1024*1024,env:{...process.env}});return clip(`${stdout||''}\n${stderr||''}`)}
...
export async function runDirectPublish(id:string,retryFailed=false,worker=false){
  ...
  for(const p of targets){
    if(DRY_RUN){
      ...
      j.results[p]={status:'succeeded',mode:'dry_run_local',externalWrite:false,message:`测试模式：已模拟${label}流程，未调用${label}外部发布代码，未发生外部平台写入。`,finishedAt:new Date().toISOString()};
      writeJob(j);
      continue;
    }
