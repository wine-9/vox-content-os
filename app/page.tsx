...
export default function Dashboard(){const items=listContentLibrary(60),pending=items.filter((x:any)=>x.statusLabel!=='已发布').slice(0,3),stats=dashboardStats();return <>
  ...
  <div style={{marginBottom:16}}><SageCard className="action-card"><div className="action-letter">1</div><h2>做一篇新内容</h2><p className="muted">先选一个话题，再和 Sinote 对齐想法、生成草稿、定稿并准备发布。</p><a className="button-link" href="/topics">开始选题 →</a></SageCard></div>
